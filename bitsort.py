...
def innersort(flist, slist, start, end, bit, order):
    ...
    if bit < 63 :  
        if start < swapPos-1:
            innersort(flist, slist, start, swapPos-1, bit+1, order)

        if swapPos < end:
            innersort(flist, slist, swapPos, end, bit+1, order)
...
def bitsort(flist):
    ...
    innersort(flist, slist, 0, swapPos-1, 1, descending)
    #Sort the sub-array of positive numbers in ascending order
    innersort(flist, slist, swapPos, len(slist)-1, 1, ascending)
